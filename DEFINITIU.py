...
while True:
    ...
    # Realizar la detección (solo boxes, no masks)
    results = model(frame, conf=0.9)  # Filtro de confianza del 90%
        ...
        for box in boxes:
            ...
            for i in range(3):
                for j in [2, 1, 0]:
                    ...
                    # Si es el facelet central (posición 5), muestreamos más arriba
                    if i == 1 and j == 1:  # Posición central en la matriz 3x3
                        offset_y = h//9  # Ajusta este valor según necesites
                        sample_y = center_y - offset_y

                        # Asegurarnos de no salirnos del facelet
                        sample_y = max(y1 + (i * h//3), sample_y)
                        
                        # Obtener color de la parte superior
                        color_bgr = get_dominant_color(frame, center_x, sample_y)
                        cv2.circle(frame, (sample_x, sample_y), 2, (0, 0, 255), -1)  # Punto rojo pequeño
                    else:
                        # Para los otros facelets, muestreamos en el centro
